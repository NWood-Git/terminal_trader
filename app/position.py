...
class Position:
    ...
    def __init__(self, **kwargs):
        ...
        # self.avg_price = kwargs.get("avg_price")#removed
    ...
    def insert(self):#works - with manual instance creation
        ...
        with sqlite3.connect(self.dbpath) as conn:
            cur = conn.cursor()
            SQL = f""" INSERT INTO {self.tablename}(account_pk, total_quantity, ticker)
                    VALUES(:account_pk, :total_quantity, :ticker)"""
            cur.execute(SQL,{'account_pk':self.account_pk, 'total_quantity':self.total_quantity,'ticker':self.ticker})
    ...
    def update(self):
        ...
        with sqlite3.connect(self.dbpath) as conn:
            ...
            total_quantity=:total_quantity, ticker=:ticker WHERE pk=:pk"""
            cur.execute(SQL,{'pk':self.pk, 'account_pk':self.account_pk, 'total_quantity':self.total_quantity,'ticker':self.ticker})
    ...
    def from_account_and_ticker(cls, account_pk, ticker):#how to feed in from account
        ...
        with sqlite3.connect(cls.dbpath) as conn:
            ...
            row = cur.fetchone()
            if row is None:
                new_pos = cls(account_pk = account_pk, total_quantity = 0, ticker = ticker)
    ...
    def all_from_account(cls, account_pk):
        with sqlite3.connect(cls.dbpath) as conn:
            ...
            cur.execute(SQL, {"account_pk":account_pk})#fetchall always returns a list if None - empty list
            rows = cur.fetchall()
            result = [cls(**row) for row in rows]
    ...
    def value(self):
        """get $ value of a pos"""
        quote = account.get_quote(self.ticker)
...
#     def save(self, ticker, total_quantity, price, account_pk):#REMOVE AVG_PRICE
#         """if the postion doesn't exist we call the insert function
#         if it does exist we call the update function
#         to insert or udpate the position in the database"""
...
#     def insert(self, ticker, quantity, price, account_pk):#works - with manual instance creation #REMOVE AVG_PRICE
...
#     def update(self,ticker, total_quantity, quantity, price, account_pk):#REMOVE AVG_PRICE
#         #not tested
